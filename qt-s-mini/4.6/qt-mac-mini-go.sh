...
nice ./configure -prefix /Users/ag/Qt/4.6.2_s_mini -debug-and-release -static \
    ...
    -no-libmng -no-libtiff -no-libjpeg -no-openssl -no-dbus -no-nis -no-iconv \
    -no-cups -no-pch -no-nas-sound -no-opengl -no-openvg -no-sm -no-xshape \
    -no-xinerama -cocoa -arch x86 -confirm-license -opensource
nice make sub-src
#make install
